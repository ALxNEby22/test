...
(function($) {
	...
	$.wysibb = function(txtArea,settings) {
		...
		this.options = {
			...
			buttons: "bold,italic,underline,strike,sup,sub,|,img,video,link,|,bullist,numlist,smilebox,|,fontcolor,fontsize,fontfamily,|,justifyleft,justifycenter,justifyright,|,quote,code,h1,h2,h3,blockquote,add_image,table,removeFormat",
			allButtons: {
				bold : {
					buttonHTML: 'html',
					excmd: 'bold',
				...
				justifycenter : {
					...
					groupkey: 'align',
					transform: {
						'<div class="position_center">{SELTEXT}</div>': '[center]{SELTEXT}[/center]'
					}
				},
				justifyleft : {
					buttonHTML: 'html',
					groupkey: 'align',
					transform: {
						'<div class="position_left">{SELTEXT}</div>': '[left]{SELTEXT}[/left]'
					}
				},
				justifyright : {
					buttonHTML: 'html',
					groupkey: 'align',
					transform: {
						'<div class="position_right">{SELTEXT}</div>': '[right]{SELTEXT}[/right]'
					}
				},
				bullist : {
					buttonHTML: 'html',
				...
				blockquote : {
					buttonText: 'html',
					transform : {
						'<blockquote>{SELTEXT}</blockquote>':"[blockquote]{SELTEXT}[/blockquote]"
					}
				},
				link : {
					buttonText: 'html',
				...
				fontsize : {
					buttonText: 'html',
					transform : {
						'<span class="wiki_size11">{SELTEXT}</span>':"[size=11]{SELTEXT}[/size]"
					}
				},
				add_image : {
					buttonText: 'html',
					transform : {
						'<img src="{IMG_URL}">':"[img]{IMG_URL}[/img]"
					}
				},
			},
			systr: {
	...
	$.wysibb.prototype = {
		...
		build: function() {
			...
			if (this.options.onlyBBmode===false) {
				...
				this.$body = $(this.strf('<div class="wysibb-text-editor" id="area" onblur="onDivBlur();" onmousedown="return cancelEvent(event);" onclick="return cancelEvent(event);" onmouseup="saveSelection();" onkeyup="saveSelection();" onfocus="restoreSelection();"></div>',{maxheight:mheight,height:height})).insertAfter(this.$txtArea);
		...
		buildButton: function(container,bn,opt) {
			...
			var $btn = $('<div class="wysibb-toolbar-btn wbb-'+bn+'">').appendTo('').append('').append(this.strf('',{title:opt.title,hotkey:hotkey}));
	...
	$.fn.insertImage = function(imgurl,thumburl) {
		...
		var code = editor.getCodeByCommand('link',{url: 'asd', text: 'as'});
		this.insertAtCursor(code);
		return editor;
	}
	$.fn.sync = function() {
...
var wiki = {
 ...
 _mark_list: function() {
  $('#editor').execCommand('bullist');
 },
 _h1: function() {
  $('#editor').execCommand('h1', {seltext: wiki._sel() ? wiki._sel() : 'Введите текст заголовка'});
 },
 _h2: function() {
  $('#editor').execCommand('h2', {seltext: wiki._sel() ? wiki._sel() : 'Введите текст заголовка'});
 },
 _h3: function() {
  $('#editor').execCommand('h3', {seltext: wiki._sel() ? wiki._sel() : 'Введите текст заголовка'});
 },
 _blockquote: function() {
  $('#editor').execCommand('blockquote', {seltext: wiki._sel() ? wiki._sel() : 'Текст цитаты'});
 },
 _add_link: function() {
  _wndBlue._show(450, {title: 'Прикрепление ссылки', text: '\
   <div id="wiki_add_link_box">\
   \
    <div class="overflow_field">\
     <div class="label">Текст ссылки:</div>\
     <div class="field"><div id="wiki_add_link_box_text_field" contenteditable="true">'+$("#editor").getSelectText()+'</div></div>\
    </div>\
    <div id="wiki_add_link_box_url" class="overflow_field">\
     <div class="label">Адрес ссылки:</div>\
     <div class="field"><input id="wiki_add_link_box_url_field" value="http://" type="text"></div>\
    </div>\
   </div>\
  ', onContent: function() {
   $('#wiki_add_link_box_text_field, #wiki_add_link_box_url_field').keydown(function(event) {
  ...
  }, _wsend: 100, _tsend: 'Прикрепить', _send: function() {
   var text = $('#wiki_add_link_box_text_field').html();
   var url = $('#wiki_add_link_box_url_field').val();
   $('#wiki_add_link_box_text_field, #wiki_add_link_box_url_field').removeClass('field_error_i');
   if(!text) {
    $('#wiki_add_link_box_text_field').addClass('field_error_i');
    $('#wiki_add_link_box_text_field').focus();
   } else if(!url.match(/^http(s)?\:\/\//)) {
    $('#wiki_add_link_box_url_field').addClass('field_error_i');
    $('#wiki_add_link_box_url_field').focus();
   } else {
    _wndBlue._closed();
    $('#editor').execCommand('link', {text: text, url: url});
   }
  }});
 },
 _fsize: function(size) {
   if(size == 11) {
    $('#editor').execCommand('fontsize_11');
   } else if(size == 12) {
    $('#editor').execCommand('fontsize_12');
   } else if(size == 13) {
    $('#editor').execCommand('fontsize_13');
   }
 }
}